...
object Day5B extends App {
  ...
  val seatIds: Seq[Int] = f.lines.map(SeatInstructions.parse).map(_.seatId).toSeq.sorted
  val missingSeatId = (seatIds.min to seatIds.max).find(!seatIds.contains(_))

  println("Day5A:")
  println(s"Missing seat id: $missingSeatId")
}
